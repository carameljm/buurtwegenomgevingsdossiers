...
def run_monitor():
    try:
        ...
        if not match.empty:
            ...
            return {
                "status": "MATCH",
                "geojson": json.loads(export_gdf.to_crs("EPSG:4326").to_json())
            }
        
        return {"status": "oke", "matches": []}
...
try {
    ...
    const output = execSync(`echo "${encoded}" | base64 -d | python3`, { timeout: 60000 }).toString();
